...
| CLM-094 | Inter-system soiling-recovery synchrony at DKA is 4.78× over random (78 pairs, Jaccard median 0.259 vs baseline 0.054) — all pairs above random; site shares cleaning events | **VERIFIED-OWN / NOVEL-FRONTIER (no prior art)** | Probe 5d memo 29 §6d | Same-site design enables this test (impossible on PVDAQ). **V2 verification 2026-05-30:** journal sweep (Deceglie 2018, Muller/Perry 2022, Micheli 2021, Jordan 2024, iScience 2021, NREL spatial-interpolation) + conference sweep (PVSC 2022-25, EU-PVSEC 2022-24, PVRW 2024-25) found no prior art using Jaccard/synchrony/K-consensus across PV fleet; closest adjacency (Meyers PVSC-53 2025 IEEE 11132652) is fleet-aware anomaly detection via regression-on-neighborhood, not set-consensus |
| CLM-095 | **Consensus-day rain alignment reveals 2 cleaning regimes:** weather-driven (K=1-11, alignment rises with K to 25% at K=8, lift 1.65×) + operational/manual (K=12-13, alignment CRASHES to 5-10% << baseline 15%, ~42 site-wide events over 12yr ~3.5/yr) — substrate-novel methodology separates natural from operational cleaning | **VERIFIED-OWN / NOVEL-FRONTIER (HARDENED, watch-item RESOLVED)** | Probe 5d memo 29 §6d | Tool for any multi-system single-site soiling analytics; explains why per-system rain-alignment (5b) was only 1.3× lift. **V2 verification 2026-05-30:** HARDENED via journal + conference sweep — candidate primary publishable methodological artifact. **Watch-item RESOLVED 2026-05-30:** IEEE 11132652 (Meyers/Dufour/Ogut PVSC-53 2025) abstract retrieved — method is quantile normalization + neighborhood regression + binary classifier for general anomaly detection; NO soiling-specific subsection, NO cleaning-event classification, NO set-based / synchrony / consensus operator. V2 stays NOVEL-FRONTIER without caveat |
...
| CLM-097 | **V2 SYNCHRONY METHODOLOGY GENERALIZES across climate + data-scale.** Applied to MA Pvoutput cluster (n=21 co-located residential, Concord/Acton MA, PVCZ 33 temperate-humid — polar opposite of DKA arid PVCZ 41). 19/21 systems usable (2 SRR NoValidIntervalError, expected at lower-soiling site). Pairwise Jaccard median 0.183 vs random null 0.091 → **2.01× random** (DKA was 4.78×). Methodology survives cross-climate (arid → temperate-humid) AND cross-data-scale (5-min on-site POA → daily PVDAQ energy + modeled NSRDB POA). | **VERIFIED-OWN (second-site)** | Probe 6 memo 32 §2-3 | Promotes V2 from one-site-novelty to genuine methodology. K-consensus crash at K=17+ (0 days) hints at operational regime even at MA — to confirm needs NOAA precipitation pull (deferred) |
