...
- 2.4 ⚠️ Partial. (a) Fresh-period null fit + per-cell Mahalanobis on first-life alpha subset [literature/06_phase2_4_5_first_results.md](literature/06_phase2_4_5_first_results.md). (b) Combined first-life + second-life pipeline ran with PPC pass (KS p=0.465) BUT exposed a fresh-period definition problem: second-life RPTs 5-7 are not truly fresh (cells at 90% SOH), pulling aging correlations into the null cov [literature/07_phase2_4_combined_result.md](literature/07_phase2_4_combined_result.md). First-life-only result remains the cleanest available; combined needs Option X1 (use first-life RPTs 1-3 as fresh for same cells across both lifecycles). Hierarchical Stan pool still to be implemented.
...
**Tasks:**
- 3.1 — Define the disagreement-onset metric. Candidate: per-cell, per-cycle joint Mahalanobis distance under the null model. Onset = first cycle where the rolling joint distance exceeds a pre-specified threshold for K consecutive cycles.
...
**Tasks:**
- 4.1 — For datasets with aging-mechanism labels (Stanford SECL has some, Zhang 2020 has EIS-based degradation pattern labels), test whether the joint inconsistency signature classifies degradation mode better than single-operator features.
